...
const loadMoreHandler = () => {
	btnLoad.value = true;
	articleStore.adminGetMoreArticles(3).finally(() => (btnLoad.value = false));
};

////GET FIRST ARTICLES
if (!articleStore.adminArticles || route.query.reload) {
	loading.value = true;
	articleStore.adminGetArticles(3).finally(() => (loading.value = false));
}
//REMVOE BY ID
const removeHandler = (articleID) => {
	loading.value = true;
	articleStore.removeById(articleID).finally(() => (loading.value = false));
};
</script>
